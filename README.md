...
- **Review state** (interval / ease / due) is written back into each note's YAML frontmatter, so
  *markdown stays the source of truth* and cross-device sync rides on the vault's own sync — no
  server, no accounts, fully offline.
