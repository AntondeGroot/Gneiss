...
export function selectDue<T extends Schedulable>(
  ...
  return {
    queue: [...reviews.slice(0, limits.reviewsPerSession), ...introduced.queue],
    heldBackReviews: Math.max(0, reviews.length - limits.reviewsPerSession),
...
function admitNew<T extends Schedulable>(
  ...
  if (runningCramDaysLeft(limits, today) === null) {
    return { ...cappedAt(fresh, limits.newPerSession), heldCrammed: 0 };
  }
  ...
  const cappedCram = cappedAt(crammed, limits.cram?.perSession ?? limits.newPerSession);
  const cappedRest = cappedAt(rest, limits.newPerSession);
...
function runningCramDaysLeft(limits: SessionLimits, today: string): number | null {
